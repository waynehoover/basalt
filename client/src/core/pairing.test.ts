...
describe("refusing a string it cannot read completely", () => {
    ...
    it("refuses a length that points past the end", () => {
        ...
        expect(() => parsePairing(PAIRING_PREFIX + base64urlEncode(reChecksum(raw)))).toThrow(/ends inside/);
    });
    ...
    it("refuses trailing rubbish that decoded cleanly", () => {
        ...
        expect(() => parsePairing(PAIRING_PREFIX + base64urlEncode(reChecksum(longer)))).toThrow(/more in it/);
    });
});
...
function reChecksum(raw: Uint8Array): Uint8Array {
    ...
    const out = raw.slice();
    out.set([(crc >>> 24) & 0xff, (crc >>> 16) & 0xff, (crc >>> 8) & 0xff, crc & 0xff], raw.length - 4);
    return out;
}
