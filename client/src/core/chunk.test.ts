...
describe("insert locality, which is the whole point", () => {
  ...
  it("an insert near the start changes almost nothing further down", () => {
    ...
    expect(fraction, `only ${(fraction * 100).toFixed(1)}% of chunks were reused`).toBeGreaterThan(
      0.95,
    );
  });
...
describe("UTF-8 boundaries", () => {
  it("never cuts a multi-byte character", () => {
    ...
    for (const c of chunkBytes(data, sizes, true)) {
      ...
      // character.
      expect((c.bytes[0]! & 0xc0) === 0x80, `chunk at ${c.offset} starts mid-character`).toBe(
        false,
      );
    }
...
describe("streaming", () => {
  it("agrees with the in-memory splitter, byte for byte", async () => {
    ...
    for (const size of [0, 1, 200, 5_000, 60_000]) {
      ...
      expect(
        streamed.map((c) => c.bytes.length),
        `size ${size} lengths`,
      ).toEqual(memory.map((c) => c.bytes.length));
    ...
    async function* blocks() {
      for (let i = 0; i < 400; i++) {
        ...
        for (let j = 0; j < block.length; j++)
          block[j] = (Math.imul(i * 1024 + j, 2654435761) >>> 24) & 0xff;
        yield block;
      }
    }
...
describe("the server's ceiling", () => {
  ...
  it("stays under the maximum even when a character straddles it", () => {
    ...
    for (const c of chunkBytes(data, sizes, true)) {
      expect(c.bytes.length, `chunk at ${c.offset} is over the maximum`).toBeLessThanOrEqual(
        sizes.max,
      );
    }
  });
