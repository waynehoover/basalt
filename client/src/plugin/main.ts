...
export default class BasaltPlugin extends Plugin {
  ...
  async deletedNotes(): Promise<DeletedList> {
    if (!this.client)
      throw new Error("not connected, so there is no way to ask what the server has");
  ...
  private render(): void {
    ...
    if (pairing) {
      ...
      new Setting(contentEl)
        ...
        .setDesc(
          "Anyone who has this string has this vault. Treat it like the passphrase it contains.",
        )
        .addButton((b) =>
          b.setButtonText("Copy pairing string").onClick(async () => {
            ...
            try {
              ...
              new Notice(
                "This device has no clipboard. The string is shown above, to copy by hand.",
              );
            }
          }),
        );
