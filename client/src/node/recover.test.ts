...
async function paired(): Promise<string> {
    ...
    const init = await cli("init", "--dir", dir, "--server", server.wsUrl, "--token", server.token, "--device", "a", "--json");
...
describe("restoring", () => {
    ...
    it("brings back an exact version by uid", async () => {
        ...
        const versions = (await cli("history", "note.md", "--dir", dir, "--json")).json()["versions"] as {
            uid: number;
            size: number;
        }[];
