...
describe("syncing real files on a real disk", () => {
    ...
    it("keeps both versions when two devices rewrite the same line", async () => {
        ...
        for (const dir of [a, b]) {
            ...
            const texts = await Promise.all(names.filter((n) => n.endsWith(".md")).map((n) => read(dir, n)));
            ...
            expect(names.some((n) => n.includes("Conflicted copy")), `${dir} has no conflict copy`).toBe(true);
        }
    }, 300_000);
...
describe("saying no clearly", () => {
    ...
    it("refuses a config whose secret is the wrong size", async () => {
        ...
        await writeFile(
            ...
            JSON.stringify({ url: "ws://x", token: "t", vaultId: "default", device: "d", secret: "AAAA" })
        );
        const r = await cli("status", "--dir", dir);
        expect(r.code).toBe(1);
