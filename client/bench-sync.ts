...
async function buildVault(dir: string): Promise<number> {
  ...
  for (let i = 0; i < COUNTS.small; i++)
    await write(pathFor("small", i, "md"), prose(sizeFor(i, 50, 50 * 1024), i));
...
async function main() {
  ...
  console.log(
    `  ${process.release.name} ${process.versions.bun ? "bun " + process.versions.bun : process.version}`,
  );
...
async function run(wire: Wire) {
  ...
  const device = async (name: string) => {
    ...
    const c = new Client({
      ...
      ...(process.env["BENCH_LOG"]
        ? {
            log: (m: string, ...r: unknown[]) =>
              console.log(`    [${name}] ${m}`, ...r.slice(0, 1)),
          }
        : {}),
    });
    clients.push(c);
  ...
  try {
    ...
    console.log(
      `  refused for good      ${up.skipped + down.skipped + daily.skipped + dailyDown.skipped}`,
    );
