...
function row(label: string, ms: number, bytes: number, extra = "") {
    ...
    console.log(`  ${label.padEnd(36)} ${rate.toFixed(0).padStart(5)} MiB/s   ${ms.toFixed(1).padStart(6)} ms  ${extra}`);
}
...
async function sealing() {
    ...
    const parts = [...chunkBytes(note(512 * 1024), sizesFor(512 * 1024, true), true)].map((c) => c.bytes);
...
async function wire(original: Uint8Array, edited: Uint8Array, sizes: ChunkSizes, isText: boolean, keys: Awaited<ReturnType<typeof deriveKeys>>) {
    const held = new Set(
...
async function realVault(path: string) {
    ...
    for (const f of files) {
        ...
        const parts = [...chunkBytes(f.data, sizesFor(f.data.length, isText), isText)].map((c) => c.bytes);
        chunks += parts.length;
        for (const c of await sealChunks(keys, parts)) onWire += c.bytes.length;
    }
    const seconds = (performance.now() - started) / 1000;

    console.log(`  first sync: ${chunks} chunks, ${fmt(onWire)} on the wire ` +
        `(${((onWire / plain) * 100).toFixed(0)}% of the plaintext)`);
    console.log(`  took ${seconds.toFixed(1)} s to chunk, compress, encrypt and name the whole vault ` +
